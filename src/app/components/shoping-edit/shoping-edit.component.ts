...
export class ShopingEditComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    // inital the slForm with the edit ingrident if we get event on ingridientIndexEdit else it inital with null
    this.subscription = this.slService.ingridientIndexEdit.subscribe(
      (index) => {
        this.editmode = true;
  ...
  onAddItem(form: NgForm) {
    // using ngForm here because its the way of the porject to give me introduce with the 2 ways of forms
